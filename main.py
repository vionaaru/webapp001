...
# Функция для создания inline-клавиатуры с двумя WebApp кнопками
def web_app_keyboard_inline():
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Тестовая страница", web_app=WebAppInfo(url="https://weather.nsu.ru"))],
            [InlineKeyboardButton(text="Запустить WebApp", web_app=WebAppInfo(url="https://vionaaru.github.io/webapp001/"))]
        ]
    )
    return keyboard
...
async def answer(web_app_mes: Message):
    ...
    print(web_app_mes.web_app_data.data)  # Вывод данных в консоль
